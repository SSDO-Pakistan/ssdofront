...
function Photosalbum(props) {
  ...
  const fetchPhotos = async (currentPage) => {
    const res = await fetch(
      `${API_URL}/api/progress-reports?populate=*&populate[0]=Report&populate[1]=Report.cover&populate[2]=Report.file&sort=rank:desc&pagination[page]=${currentPage}&pagination[pageSize]=${limit}`
    );
...
export async function getStaticProps() {
  const res = await fetch(
    `${API_URL}/api/newsletters?populate=*&sort=rank:desc&pagination[page]=1&pagination[pageSize]=10`
  );
