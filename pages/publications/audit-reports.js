...
function Photosalbum(props) {
  ...
  const fetchPhotos = async (currentPage) => {
    const res = await fetch(
      `${API_URL}/api/audit-reports?populate[0]=Report&populate[1]=Report.cover&populate[2]=Report.file&sort=rank:asc&pagination[page]=${currentPage}&pagination[pageSize]=${limit}`
    );
...
export async function getStaticProps() {
  const res = await fetch(
    `${API_URL}/api/audit-reports?populate[0]=Report&populate[1]=Report.cover&populate[2]=Report.file&sort=rank:asc&pagination[page]=1&pagination[pageSize]=10`
  );
