            ...
            <div className=" h-auto col-md-12">
                <div className="block-title-6 text-center">
                    <h4 className="h5 border-primary ">
                        <span className="bg-primary text-white">HighLights details</span>
                    </h4>
                </div>
                <div className="">
                    {data && (
...
export async function getServerSideProps({ query: { slug } }) {
    const res = await fetch(
        `${API_URL}/api/posts?&populate=*&filters[type][$eq]=Highlights&filters[slider][$eq]=false&filters[slug][$eq]=${slug}`
    );
